---
title: "Demultiplex hashtags in SLN111_D2 (Lane 12)"
...
outs_dir <- "/data/yosef2/users/zsteier/TotalSeq/20190814_BioLegend_ZRS08/cellranger_out/12_B6_spl_LN_r5_111/outs/"
...
seurat_obj <- HTODemux(seurat_obj, assay = "HTO", positive.quantile = 0.9995, kfunc = "kmeans") # default is clara
...
Parameters: kmeans, 0.9995 (quantile above negative cells in NB distribution)

Cutoff for HTO-B6-LN-r4-206-A0301 : 19 reads
Cutoff for HTO-B6-spl-r4-206-A0302 : 23 reads
...
#### Note: hashtag order is the opposite of spleenLN206
...
tissue <- tibble(Cell_BC = row.names(seurat_obj@meta.data), hash.ID = seurat_obj@meta.data$hash.ID) %>%
  mutate(Hash_ID = str_replace(str_replace(hash.ID, "HTO-B6-LN-r4-206-A0301", "Spleen"), "HTO-B6-spl-r4-206-A0302", "Lymph_Node")) %>%
  select(-hash.ID)

table(tissue$Hash_ID)

# Save metadata
write_csv(tissue, "spleenLN111_day2/hashtag_labels.csv")
...
saveRDS(seurat_obj, "spleenLN111_day2/seurat_obj.rds")

```


## Create concatenated labels for two days SLN111

```{r Concatenate and save labels}
day1 <- read_csv("/data/yosef2/users/zsteier/totalVI/totalVI_journal/notebooks_zoe/Seurat_demux/spleenLN111_day1/hashtag_labels.csv")
day2 <- read_csv("/data/yosef2/users/zsteier/totalVI/totalVI_journal/notebooks_zoe/Seurat_demux/spleenLN111_day2/hashtag_labels.csv")

concat_days <- rbind(day1, day2)

```

```{r Save concatenated output}
# This is the result that will be used for downstream analysis
write_csv(concat_days, "/data/yosef2/users/zsteier/totalVI/totalVI_journal/data/metadata/spleenLN111/hashtag_labels.csv")
```